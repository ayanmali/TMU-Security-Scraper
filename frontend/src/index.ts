...
async function changePage(dir: string): Promise<void> {
...
function createIncidentCard(incident: Incident): string {
    return `
        <div class="incident-card">
            ...
            <p><strong>Date of Incident:</strong> ${moment.utc(incident.date_of_incident || incident.dateofincident).format('MMMM D, YYYY')}</p>
...
async function fetchAllIncidents(): Promise<void> {
    ...
    try {
        ...
        if (container) { container.innerHTML = data.results.map(createIncidentCard).join(''); }
    } catch (error) {  
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('#recommendations-tab button')?.addEventListener('click', getRecommendations);
