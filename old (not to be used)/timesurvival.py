"""
Implements a neural network to take an incident type and location, and predict
how much time will pass until another incident will occur (regardless of type/location).
(WIP)
"""
...
class IncidentDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        X = self.features[idx:idx + 1]
        # y = self.target[idx + 1 : idx + self.sequence_length + 1]
        y = self.target[idx:idx + 1]
